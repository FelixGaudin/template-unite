> **Note** — Ce fichier est le README d’origine du template graphique Les Scouts.
> Ce dépôt a été transformé en **site d’unité** : le site occupe désormais la
> racine et se modifie depuis `/admin`. Voir [`README.md`](README.md).
>
> Le contenu ci-dessous reste utile pour comprendre et faire évoluer la charte
> graphique (fichiers `scss/`, `css/`, `images/`, `fonts/`).
...
### Un site complet, modifiable sans connaissances techniques

C’est ce que contient désormais ce dépôt : voir [`README.md`](README.md).
